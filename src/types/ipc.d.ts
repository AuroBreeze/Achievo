...
declare global {
  interface Window {
    api?: {
      ...
      getConfig(): Promise<{ openaiApiKey?: string; repoPath?: string; lastProcessedCommit?: string | null; lastSummaryDate?: string | null; aiProvider?: 'openai' | 'deepseek' | 'custom'; aiModel?: string; aiBaseUrl?: string; aiApiKey?: string; quoteFontSize?: number; quoteEnabled?: boolean; quoteRefreshSeconds?: number; quoteLetterSpacing?: number }>; 
      setConfig(cfg: { openaiApiKey?: string; repoPath?: string; lastProcessedCommit?: string | null; lastSummaryDate?: string | null; aiProvider?: 'openai' | 'deepseek' | 'custom'; aiModel?: string; aiBaseUrl?: string; aiApiKey?: string; quoteFontSize?: number; quoteEnabled?: boolean; quoteRefreshSeconds?: number; quoteLetterSpacing?: number }): Promise<void>;
      ...
      statsGetToday(): Promise<{ date: string; insertions: number; deletions: number; baseScore: number; trend: number; summary?: string | null; aiScore?: number | null; localScore?: number | null; progressPercent?: number | null; aiModel?: string | null; aiProvider?: string | null; aiTokens?: number | null; aiDurationMs?: number | null; chunksCount?: number | null; lastGenAt?: number | null; createdAt: number; updatedAt: number }>;
      statsGetRange(payload: { startDate: string; endDate: string }): Promise<Array<{ date: string; insertions: number; deletions: number; baseScore: number; trend: number; summary?: string | null; aiScore?: number | null; localScore?: number | null; progressPercent?: number | null; aiModel?: string | null; aiProvider?: string | null; aiTokens?: number | null; aiDurationMs?: number | null; chunksCount?: number | null; lastGenAt?: number | null; createdAt: number; updatedAt: number }>>;
